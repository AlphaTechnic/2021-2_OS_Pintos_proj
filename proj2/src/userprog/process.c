...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  char* ptr = strtok_r(fn_copy, " ", &nxt);
  char* thread_name = ptr;
  while (ptr != NULL){
    argv[argc++] = ptr;
    ptr = strtok_r(NULL, " ", &nxt);
  }
  ///////////////////////////
...
void push_to_stack(char** argv, void **esp, int argc){
  int tot = 0;
  char* ADDR[256];
  // argv
