...
void list_handler(char *command, int flag)
{
    ...
    switch (flag) {
        ...
        case DELETE:
            idx = command[0] - '0';
            for (cur = list_begin(L[idx]); cur != list_end(L[idx]); cur = nxt) {
                nxt = list_next(cur);
    ...
    if (flag == OTHERS) {
        ...
        else if (strncmp(param[0], "min", 3) == 0){
            void *aux;
        ...
        else if (strncmp(param[0], "reverse", 7) == 0){
            list_reverse(L[idx]);
        }
...
void hash_handler(char *command, int flag)
{
    ...
    if (flag == OTHERS) {
        ...
        else if (strncmp(param[0], "clear", 5) == 0) {
        ...
        else if (strncmp(param[0], "replace", 7) == 0) {
            int val = str2int(param[2]);
            struct hash_item *new_item = (struct hash_item *) malloc(sizeof(struct hash_item));
            new_item->data = val;
...
void bitmap_handler(char *command, int flag){
    ...
    if (flag == OTHERS) {
        ...
        if (strncmp(param[0], "mark", 4) == 0) {
            index1 = str2int(param[2]);
            bitmap_mark(B[idx], index1);
        }
        else if (strncmp(param[0], "all", 3) == 0) {
            // all set true?
            index1 = str2int(param[2]);
            num = str2int(param[3]);
        ...
        else if (strncmp(param[0], "any", 3) == 0) {
            // any set true?
            index1 = str2int(param[2]);
            num = str2int(param[3]);
        ...
        else if (strncmp(param[0], "contains", 8) == 0) {
            // [a]부터 n개가 contain true?
            index1 = str2int(param[2]);
            num = str2int(param[3]);
            bool val = false;
            if (strncmp(param[4], "true", 4) == 0) {
                val = true;
            }
        ...
        else if (strncmp(param[0], "count", 5) == 0) {
            index1 = str2int(param[2]);
            num = str2int(param[3]);
            bool val = false;
            if (strncmp(param[4], "true", 4) == 0) {
        ...
        else if (strncmp(param[0], "dump", 4) == 0) {
            bitmap_dump(B[idx]);
        }
        else if (strncmp(param[0], "expand", 6) == 0) {
            // bitmap_expand bm0 4
            num = str2int(param[2]);
            bitmap_expand(B[idx], num);
        }
        else if (strncmp(param[0], "set_multiple", 12) == 0) {
            // bitmap_set_multiple bm0 0 4 true
            index1 = str2int(param[2]);
            num = str2int(param[3]);
            bool val = false;
            if (strncmp(param[4], "true", 4) == 0) val = true;

            bitmap_set_multiple(B[idx], index1, num, val);
        }
        else if (strncmp(param[0], "set_all", 7) == 0) {
            bool val = false;
            if (strncmp(param[2], "true", 4) == 0) val = true;

            bitmap_set_all(B[idx], val);
        }
        else if (strncmp(param[0], "set", 3) == 0) {
            index1 = str2int(param[2]);
            bool val = false;
            if (strncmp(param[3], "true", 4) == 0) val = true;

            bitmap_set(B[idx], index1, val);
        }
        else if (strncmp(param[0], "flip", 4) == 0){
            // bitmap_flip bm0 4
            index1 = str2int(param[2]);
            bitmap_flip(B[idx], index1);
        }
        else if (strncmp(param[0], "none", 4) == 0) {
            index1 = str2int(param[2]);
            num = str2int(param[3]);
        ...
        else if (strncmp(param[0], "reset", 5) == 0){
            // bitmap_reset bm0 0
            index1 = str2int(param[2]);
            bitmap_reset(B[idx], index1);
        }
        else if (strncmp(param[0], "scan_and_flip", 13) == 0) {
        ...
        else if (strncmp(param[0], "scan", 4) == 0) {
        ...
        else if (strncmp(param[0], "test", 4) == 0) {
            // bitmap_test bm0 4
            index1 = str2int(param[2]);
